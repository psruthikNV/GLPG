...
int main() {
    ...
    char *data;
    char *psData;
    const char *compiledVSPath = "C:/repos/GLPG/build/d3d11/03_model_matrix_vs";
    const char *compiledPSPath = "C:/repos/GLPG/build/d3d11/03_model_matrix_ps";
    std::fstream fs;
    std::streampos size;
    ...
    ConstantBufferData vsConstantBufferData;
    std::memset(&vsConstantBufferData, 0, sizeof(ConstantBufferData));
    ...
    while ((event = eventLoop.GetEvent()) != GLPG::GLPGEvent::Key_Escape) {
        GLPG::mat4x4_f modelMatrix;
        GLPG::vec3_f translateVector = {dist(gen),
                                        dist(gen),
                                        dist(gen)};
