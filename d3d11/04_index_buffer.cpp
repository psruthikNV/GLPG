...
int main() {
    ...
    ret = pD3d11device->CreateBuffer(&idxBufferDesc, &idxInitData, &pIndexBuffer);
    if (ret != S_OK) {
        std::cerr << "Failed to create Index Buffer\n";
        std::cerr << "Ret: " << std::hex << ret << std::dec << "\n";
    } else {
        std::cerr << "Created Index Buffer\n";
    }
    ...
    pD3d11context->IASetIndexBuffer(pIndexBuffer, DXGI_FORMAT_R32_UINT, 0U);
