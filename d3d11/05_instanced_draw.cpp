...
int main() {
    ...
    ret = pD3d11device->CreateInputLayout(iaDesc, 2U, data, size, &pInputLayout);
    if (ret != S_OK) {
    ...
    VtxInput translateVectors[10];
    std::memset(translateVectors, 0, sizeof(translateVectors)); 
    for (uint32_t idx = 0U; idx < 10U; idx++) {
        translateVectors[idx].x = dist(gen);
        translateVectors[idx].y = dist(gen);
        std::cout << "Translate Vector " << idx << ": " << translateVectors[idx].x << " " << translateVectors[idx].y << " " << translateVectors[idx].z << "\n";
    }
    ...
    D3D11_SUBRESOURCE_DATA instanceBufferInitData = {
        translateVectors,
        0,
        0
    };
    ID3D11Buffer *pInstanceBuffer;
    ret = pD3d11device->CreateBuffer(&instanceBufferDesc, &instanceBufferInitData, &pInstanceBuffer);
    if (ret != S_OK) {
        std::cerr << "Failed to create instance buffer\n";
        return -1;
    } else {
        std::cerr << "Created instance buffer\n";
    }
    ...
    pD3d11context->IASetVertexBuffers(1, 1, &pInstanceBuffer, &stride, &offset);
    ...
    while ((event = eventLoop.GetEvent()) != GLPG::GLPGEvent::Key_Escape) {
        ...
        pD3d11context->DrawInstanced(3, 10, 0, 0);
